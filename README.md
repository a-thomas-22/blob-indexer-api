...
- `GET /api/v1/blob/{txHash}/inclusion?network=mainnet` - The transaction's inclusion timeline: `first_seen_at` and `time_to_inclusion_ms` as on `/blob/{txHash}`, the `included` block with its builder and blob occupancy (null while pending), and `skipped` — every block that arrived while the transaction sat in our node's pool and did not include it, oldest first, each with its builder, occupancy, blob base fee, `waited_ms` and the `reason` the indexer classified the miss under (`eligible`, `too_recent`, `nonce_gap`, `priced_out_blob_fee`, `priced_out_exec_fee`, `no_room`). `window` bounds the blocks the transaction waited through and says how many of them classified their pending pool (`snapshot_blocks`), which is the most that can appear in `skipped`; `skipped_blocks` and `eligible_skipped_blocks` count the retained history even when the list is truncated to its 500 most recent entries (`skipped_truncated`). `window` is null only when the transaction was never seen pending and no block recorded it; a pending transaction newer than every indexed block gets the empty span past the newest block. Only `eligible` says anything about the builder, and only as "visible to our node and not included": the per-block detail exists solely for blocks indexed live and is pruned after `indexer.candidate_retention`
...
- Tools loop back into the REST handlers in-process, so they return exactly the `/api/v1` payloads (`{"data": ..., "meta": ...}`) with the same caching and validation. Tools: `get_blob_market_overview` (one-call snapshot), `list_networks`, `get_indexer_status`, `get_blob_pricing`, `get_mempool_pressure`, `get_blob_stats`, `get_rolling_stats`, `get_blob_market_chart`, `get_attribution_usage_chart`, `get_cost_comparison_chart`, `get_blob_tips_chart`, `get_builder_share_chart`, `get_builders`, `get_builder`, `get_top_blob_users`, `get_entity`, `get_blob_records`, `get_latest_blobs`, `get_mempool_blobs`, `get_blob_replacements`, `get_block`, `get_blob_by_tx_hash`, `get_blob_inclusion`, `get_blob_by_versioned_hash`, `search`. One prompt, `explain_blob_market`, walks a model through a plain-language market explanation.
...
`MCP_API_KEYS` is a comma-separated list of `name:secret` entries. When `mcp.keys` declares keys, the file owns the permission model and the env var may only supply secrets: every entry must name a declared key, may not carry a tool list, and an undeclared name is a load error. When `mcp.keys` is empty, the env var is the whole configuration and entries may include an allowlist as `name:secret:tool1|tool2`. Malformed entries are load errors rather than being dropped, including a trailing `:` with no tools after it (an empty allowlist would silently mean "every tool"), and secrets may not contain `:` or `,`. Secrets must be at least 16 characters. Enabling MCP without any key is a startup error. `mcp.path` must be a literal, canonical path (no chi patterns, no `.`/`..` segments, at most 512 characters) outside `/api`, `/metrics`, `/swagger`, and `/asyncapi.yaml`.
...
indexer:
  ...
  # tx_index from the same fetch. Walks newest first so the recent windows
  # /builders serves fill before older history. Resumable and
  # non-destructive: an existing builder row is never overwritten and
  # nothing is deleted. The pause throttles the RPC load it adds next to
  # live indexing.
