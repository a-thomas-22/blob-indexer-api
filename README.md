...
4. App releases publish Docker images to Harbor at `registry.ahkc.win/${GITHUB_REPOSITORY}-api` and `registry.ahkc.win/${GITHUB_REPOSITORY}-indexer`; chart releases package and push the Helm chart to `oci://registry.ahkc.win/${GITHUB_REPOSITORY_OWNER}/charts`
