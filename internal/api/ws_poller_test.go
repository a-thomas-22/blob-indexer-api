...
type pollerBlockDB struct {
	...
	buildersErr   error
	// buildersErrN limits buildersErr to the first N builder reads, which is
	// how a transient failure that the in-request retry clears is staged;
	// zero fails every read.
	buildersErrN     int
	buildersErrCalls int
}
...
func (p *pollerBlockDB) mock() *mockDB {
	return &mockDB{
		...
		builderSelectFn: func(_ context.Context, dest interface{}, _ string, args ...interface{}) error {
			...
			p.buildersErrCalls++
			if p.buildersErr != nil && (p.buildersErrN == 0 || p.buildersErrCalls <= p.buildersErrN) {
...
// A builder read that fails once is retried inside the same broadcast: a
// client cannot tell "no builder row yet" from "the query blew up", and the
// event is never repeated, so the retry has to happen before the broadcast.
func TestPoller_BroadcastRetriesBuilderQueryError(t *testing.T) {
	hub := NewHub()
	go hub.Run()
	defer hub.Stop()
	client := registerTestClient(t, hub)
	...
	nums := newBlockNumbers(t, events)
	if len(nums) != 1 || nums[0] != 101 {
		t.Fatalf("expected the block to broadcast after the retry, got %v", nums)
	}
	var builder *BlockBuilderResponse
	for _, event := range events {
		if event.Type != EventNewBlock {
	...
	if builder == nil || builder.Key != "beaverbuild" {
		t.Fatalf("the retried read must fill the builder in, got %+v", builder)
	}
}

// When the retry fails too, the block is not broadcast without its builder:
// it is unmarked so the trailing scan re-broadcasts it complete, exactly how
// a metrics or blob failure is handled.
func TestPoller_BroadcastDefersOnPersistentBuilderQueryError(t *testing.T) {
	hub := NewHub()
	go hub.Run()
	defer hub.Stop()
	client := registerTestClient(t, hub)
	...
	if nums := newBlockNumbers(t, drainEvents(client, 50*time.Millisecond)); len(nums) != 0 {
		t.Fatalf("a builder read that keeps failing must not broadcast a builder-less block, got %v", nums)
	}
	if _, seen := st.seen[101]; seen {
		t.Fatal("failed broadcast must unmark the block so the scan retries it")
	}

	// The error clears; the catch-up scan retries the block, builder and all.
	...
	nums := newBlockNumbers(t, drainEvents(client, 50*time.Millisecond))
	if len(nums) != 1 || nums[0] != 101 {
...
func TestPoller_BroadcastUsersUpdate_Success(t *testing.T) {
	...
	db := &mockDB{
		selectFn: func(ctx context.Context, dest interface{}, query string, args ...interface{}) error {
			...
			if out, ok := dest.(*[]models.BlobUserGroupStats); ok {
				if query != queryTopBlobUserGroupsAll {
					t.Errorf("expected grouped broadcast to use queryTopBlobUserGroupsAll")
				}
				*out = []models.BlobUserGroupStats{
					{Key: "base", Name: "Base", Addresses: pq.StringArray{"0xdead", "0xbeef"}, BlobCount: 7, TotalCostWei: "0"},
				}
			}
			return nil
		},
	...
	for _, e := range events {
		...
		case EventUsersGroupedUpdate:
			...
			if err := json.Unmarshal(data, &rows); err != nil {
				t.Fatalf("failed to decode grouped users payload: %v", err)
			}
...
func TestPoller_BroadcastUsersUpdate_GroupedQueryError(t *testing.T) {
	hub := NewHub()
	go hub.Run()
	defer hub.Stop()
	client := registerTestClient(t, hub)
	...
	db := &mockDB{
		selectFn: func(ctx context.Context, dest interface{}, query string, args ...interface{}) error {
			if _, ok := dest.(*[]models.BlobUserGroupStats); ok {
				return fmt.Errorf("grouped users query error")
			}
			return nil
		},
	}
	...
	// The grouped variant failing must not cost the per-address broadcast.
	events := drainEvents(client, 50*time.Millisecond)
	if countEvents(events, EventUsersUpdate) != 1 {
